...
class InputanDokterController extends Controller
{
     public function index(Request $request,$no_rawat)
    {
        ...
        $data = DB::table('hasil_inputan_dokter_mcu2')->where('no_rawat', $no_rwt)->first();
      
        return view('inputanhasildokter::components.hasil.rdmp.index',[
            'reg' => $reg, 
            'pasien' => $pasien, 
            'data' => $data,
            'no_rawat' => $no_rawat,
    ...
    {
        $no_rwt = Crypt::decrypt($no_rawat);
        $hasil = DB::table('hasil_inputan_dokter_mcu2')->where('no_rawat', $no_rwt)->orderByDesc('id')->first();
        $reg = DB::table('reg_periksa')->where('no_rawat', $no_rwt)->first();
        $pasien = DB::table('pasien')->where('no_rkm_medis', $reg->no_rkm_medis)->first();

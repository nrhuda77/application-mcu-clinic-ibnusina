...
namespace Modules\InputanHasilDokter\Http\Controllers\Umum;
...
class CrudInputanDokterController extends Controller
{
      public static function store(Request $request) {
        try {
            ...
            DB::table('hasil_inputan_dokter_mcu')->insert([
                ...
                'hasil_kategori' => $request->hasil_kategori,
                'catatan_hasil' => $request->saran,
    ...
    public static function update(Request $request) {
        try{
            ...
            $data = request()->all();
            $data = request()->except(['_token']);
            DB::table('hasil_inputan_dokter_mcu')->where('no_rawat',$no_rwt)->where('jenis', 'hasil')->update($data);
    ...
    public static function destroy($no_rawat) {
        ...
        DB::table('hasil_inputan_dokter_mcu')->where('no_rawat', $no_rwt)->where('jenis', 'hasil')->delete();
        return response()->json(['status' => 'success']);
    }
  ...
  
    {
        ...
        $exists = DB::table('hasil_inputan_dokter_mcu')->where('no_rawat', $no_rawat)->where('jenis', 'hasil')->exists();
    ...
    public static function pdf_Preview_temuan($no_rawat)
    {
        ...
        $pdf->SetFont('Times', '', 11.5);
        $pdf->MultiCell(100, 4, $inputan?->hasil, 0, 0); // label
    ...
    public static function pdf_Preview_saran($no_rawat)
    {
        ...
        $pdf->SetXY(50, $pdf->GetY()+5.5);
        $pdf->SetFont('Times', '', 11.5);
        $pdf->MultiCell(100, 4, $inputan?->catatan_hasil, 0, 0); // label

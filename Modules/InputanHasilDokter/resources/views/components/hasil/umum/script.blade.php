...
$('#summernote2').on('input', function () {
    let contents2 = $(this).val();
    autoSaveContent2(contents2); // panggil fungsi autosave
});



$('[name="hasil_kategori"], [name="kesimpulan"]').on('change input', function () {
    ...
    timeout1 = setTimeout(function () {
      ...
      zIndex: 9999 // Gerakkan elemen 20px ke atas
        ...
        $.ajax({
            ...
            success: function (res) {
                $('#pdfFrame').attr('src', '/pdf-preview-temuan-umum/' + '{{ $norawat }}');
                 Notiflix.Block.remove('.blocking');
            }
        });
    }, 400);
...
let timeout2 = null;
function autoSaveContent2(content) {
    ...
    timeout2 = setTimeout(function () {
      ...
      zIndex: 9999 // Gerakkan elemen 20px ke atas
        ...
        $.ajax({
            url: "/auto-save-saran-umum",
            method: "POST",
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                catatan_hasil: content,
                hasil_kategori: $('[name="hasil_kategori"]').val(),
                kesimpulan: $('[name="kesimpulan"]').val(),
                no_rawat: '{{ $norawat }}',
                no_reg: '{{ $reg->no_reg }}',
            ...
            success: function (res) {
                $('#pdfFrame2').attr('src', '/pdf-preview-saran-umum/' + '{{ $norawat }}');
                 Notiflix.Block.remove('.blocking');
            }
        });
    }, 400);
...
cancelButtonBackground: '#a9a9a9',
  ...
  function() {
    ...
    $.ajax({
        url : "/delete-hasil-inputan-dokter-umum/" + no_rawat,
        type: "GET",
        dataType: "JSON",

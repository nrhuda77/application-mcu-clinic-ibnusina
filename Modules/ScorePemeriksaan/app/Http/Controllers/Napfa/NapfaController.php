...
class NapfaController extends Controller
{
     public function index(Request $request,$no_rawat)
    {
        ...
        $data = DB::table('score_pemeriksaan_napfa')->where('no_rawat', $no_rwt)->first();

        $path = $request->segment(1);         
      
        return view('scorepemeriksaan::components.napfa.index',[
            'reg' => $reg, 
            'pasien' => $pasien, 
            'data' => $data,
            'no_rawat' => $no_rawat,
    ...
    {
        $no_rwt = Crypt::decrypt($no_rawat);
        $score = DB::table('score_pemeriksaan_napfa')->where('no_rawat', $no_rwt)->orderByDesc('id')->first();
        $reg = DB::table('reg_periksa')->where('no_rawat', $no_rwt)->first();
        $pasien = DB::table('pasien')->where('no_rkm_medis', $reg->no_rkm_medis)->first();
    ...
    public function store(Request $request) {
     return CrudNapfaController::store($request);
    }

    public function update(Request $request) {
        return CrudNapfaController::update($request);
    }

    public function destroy($no_rawat) {
        return CrudNapfaController::destroy($no_rawat);
    }
}
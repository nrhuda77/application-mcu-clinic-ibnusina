...
namespace Modules\InputanDokterAudiogram\Http\Controllers;
...
class InputanDokterAudiogramController extends Controller
{
    ...
    $jns = DB::table('hasil_inputan_dokter_mcu')->where('no_rawat', $val->no_rawat)->where('jenis', 'audiogram')->first();
                      ...
                      <i class="ms-1 icon-base ti tabler-brand-juejin icon-15px"></i></button>
                          <div class="dropdown-menu">
                            <a class="dropdown-item text-primary" onclick="detail(\''. $no_rawat.'\')"><i class="icon-base ti tabler-list-details me-1"></i> Detail</a>
                             <a class="dropdown-item text-warning" href="/detail-hasil-inputan-dokter-audiogram/'. $no_rawat.'"><i class="icon-base ti tabler-app-window me-1"></i>Input Audiogram</a>
                           </div>
                </div>

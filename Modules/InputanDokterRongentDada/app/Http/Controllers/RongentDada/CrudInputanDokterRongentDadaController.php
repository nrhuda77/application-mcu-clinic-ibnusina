...
class CrudInputanDokterRongentDadaController extends Controller
{
    public static function store(Request $request) {
        try {
            $data = request()->all();
            $data = request()->except(['_token']);
            DB::table('hasil_radiologi')->insert([
    ...
    public static function update(Request $request) {
        try{
            ...
            $data = request()->all();
            $data = request()->except(['_token']);
            DB::table('hasil_radiologi')->where('no_rawat',$no_rwt)->update($data);
    ...
    public static function destroy($no_rawat) {
        ...
        DB::table('hasil_radiologi')->where('no_rawat', $no_rwt)->delete();
        return response()->json(['status' => 'success']);
    }


    public static function auto_save_rongentdada(Request $request)
  
    {
          $no_rawat = Crypt::decrypt($request->no_rawat);
        // Cek apakah sudah ada data user ini, update atau insert
        $exists = DB::table('hasil_radiologi')->where('no_rawat', $no_rawat)->exists();
    ...
    public static function pdf_Preview_rongentdada($no_rawat)
    {
        $norawat = Crypt::decrypt($no_rawat);
        $data = DB::table('hasil_radiologi')->where('no_rawat', $norawat)->first();
        ...
        $pdf = Pdf::loadView('inputandokterrongentdada::components.pdf', 
        [
            'content' => $content,
            'kop' => $kop,
            'watermark' => $watermark

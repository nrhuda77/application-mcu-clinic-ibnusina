...
class HasilElektroKardiogramController extends Controller
{
...
$pdf->Image($tempFile, 30, 75, 140);
     ...
     public static function index($pdf, $norwt)
    {
        ...
        $hsl_ekg = DB::table('hasil_elektrokardiogram')->where('no_rawat', $norwt)->first();
         if($hsl_ekg == null){
    ...
    $pdf->SetFont('Times', 'B', 13.5);
     $word = "HASIL PEMERIKSAAN ELEKTROKARDIOGRAM";
     self::header($pdf);
     self::head_periksa($pdf,$pasien,$reg,$dokter,$word);

     $pdf->Text(26, 78, "Hasil");  
      $newPath = str_replace("", "-", $hsl_ekg->hasil ?? '');

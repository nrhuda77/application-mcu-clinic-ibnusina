...
class HasilRongentDadaController extends Controller
{
...
$pdf->Image($tempFile, 30, 75, 140);
    ...
    public static function index($pdf, $norwt)
    {
        ...
        $hsl_rgn_dada = DB::table('hasil_radiologi')->where('no_rawat', $norwt)->first();
         if($hsl_rgn_dada == null){
    ...
    $pdf->SetFont('Times', 'B', 13.5);
     $word = "HASIL PEMERIKSAAN ROTGEN DADA";
     self::header($pdf);
     self::head_periksa($pdf,$pasien,$reg,$dokter,$word);

     $pdf->Text(26, 78, "Hasil");  
      $newPath = str_replace("", "-", $hsl_rgn_dada->hasil ?? '');

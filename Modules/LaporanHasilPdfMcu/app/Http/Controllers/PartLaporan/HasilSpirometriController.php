...
class HasilSpirometriController extends Controller
{
...
$pdf->Image($tempFile, 30, 75, 140);
     ...
     public static function index($pdf, $norwt)
    {
        ...
        $hsl_spirometri = DB::table('hasil_inputan_dokter_mcu')->where('jenis', 'spirometri')->where('no_rawat', $norwt)->first();
         if($hsl_spirometri == null){
    ...
    $pdf->SetFont('Times', 'B', 13.5);
     $word = "HASIL PEMERIKSAAN SPIROMETRI";
     self::header($pdf);
     self::head_periksa($pdf,$pasien,$reg,$dokter,$word);

     $pdf->Text(26, 78, "Hasil");  
      $newPath = str_replace("", "-", $hsl_spirometri->hasil ?? '');
